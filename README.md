...
up via the `L10n` helper. Dutch (`nl`), German (`de`), French (`fr`), Spanish
(`es`), and Chinese — Simplified and Traditional — translations are included;
add another language by dropping in a new `.lproj` folder and translating the
values — it then also appears as a choice in Settings.
